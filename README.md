...
│                         API LAYER                                    │
│  ┌─────────────────────────────────────────────────────────────────┐ │
│  │  Express Backend (Railway / Docker)                             │ │
│  │  ┌──────────┬──────────┬───────────┬──────────┬──────────────┐ │ │
...
│
├── Dockerfile                     # 🐳 Root multi-stage Docker build (Railway)
...
**Backend Base URL**: `https://your-backend.up.railway.app/api`
...
| **Relational DB** | PostgreSQL (Railway plugin) via `pg` Pool | Gold rates, products, inventory |
...
| **Backend Hosting** | Railway | Docker container hosting |
...
> **Security:** Never commit `.env` files. Set all secrets via Railway/Cloudflare dashboards.
...
railway login
railway link
railway up  # Deploys using root Dockerfile
```
