...
export default function DashboardScreen() {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const goldRateData = goldRateResponse.data.data;
            setData({
    ...
    return (<ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh}/>}>
      ...
      <View style={styles.metricsGrid}>
        <MetricCard title="Today's Sales" value={formatCurrency(data.todaySales)} subtitle={`${data.todayTransactions} transactions`} color="#10b981" icon="💰"/>
        
        <MetricCard title="Stock Value" value={formatCurrency(data.stockValue)} subtitle={`${data.stockReport?.availableProducts || 0} available items`} color="#3b82f6" icon="📦"/>
        
        <MetricCard title="Pending Dues" value={formatCurrency(data.pendingDues)} subtitle="From customers" color="#f59e0b" icon="⏰"/>
        
        <MetricCard title="Today's Purchases" value={formatCurrency(data.salesReport?.totalPurchases || 0)} subtitle="Inventory purchases" color="#8b5cf6" icon="🛍️"/>
      </View>

      {/* Sales Breakdown */}
      {data.salesReport && (<View style={styles.card}>
          <Text style={styles.cardTitle}>Payment Methods</Text>
          <View style={styles.breakdownContainer}>
            <BreakdownItem label="Cash" value={formatCurrency(data.salesReport.cashSales)} percentage={calculatePercentage(data.salesReport.cashSales, data.todaySales)}/>
            <BreakdownItem label="Card" value={formatCurrency(data.salesReport.cardSales)} percentage={calculatePercentage(data.salesReport.cardSales, data.todaySales)}/>
            <BreakdownItem label="UPI" value={formatCurrency(data.salesReport.upiSales)} percentage={calculatePercentage(data.salesReport.upiSales, data.todaySales)}/>
          </View>
        </View>)}

      {/* Stock Summary */}
      {data.stockReport && (<View style={styles.card}>
...
const MetricCard = ({ title, value, subtitle, color, icon }) => (<View style={[styles.metricCard, { borderLeftColor: color }]}>
    <Text style={styles.metricIcon}>{icon}</Text>
    <Text style={styles.metricTitle}>{title}</Text>
    <Text style={[styles.metricValue, { color }]}>{value}</Text>
    <Text style={styles.metricSubtitle}>{subtitle}</Text>
  </View>);
const BreakdownItem = ({ label, value, percentage }) => (<View style={styles.breakdownItem}>
...
const ActionButton = ({ icon, label }) => (<TouchableOpacity style={styles.actionButton}>
