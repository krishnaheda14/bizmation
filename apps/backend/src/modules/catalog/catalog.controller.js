...
export function catalogRouter(db) {
    ...
    router.get('/items/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const result = await db.query('SELECT * FROM catalog_items WHERE id = $1', [id]);
    ...
    router.post('/items', upload.single('image'), async (req, res) => {
        try {
            ...
            const { name, description, category, metalType, purity, weightGrams, makingCharges, hsnCode, tags, } = formData;
    ...
    router.delete('/items/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const result = await db.query('DELETE FROM catalog_items WHERE id = $1 RETURNING *', [id]);
            if (result.rows.length === 0) {
                return res.status(404).json({
                    success: false,
                    error: 'Catalog item not found',
