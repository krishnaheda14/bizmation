...
export class InventoryService {
    constructor(db, goldRateService) {
        this.db = db;
     ...
     */
    async createMetalLot(shopId, data) {
        const metalLot = {
            id: this.generateId(),
            ...data,
            shopId,
     ...
     * Get metal lot by ID
     */
    async getMetalLotById(id, shopId) {
        const query = `
     ...
     */
    async getMetalLots(shopId, filters) {
        let query = `
      SELECT * FROM metal_lots
     ...
     */
    async updateMetalLotWeight(id, shopId, weightUsed) {
        const query = `
      UPDATE metal_lots
    ...
    async createProduct(shopId, data) {
        ...
        if (product.metalLotId) {
            await this.updateMetalLotWeight(product.metalLotId, shopId, product.netWeightGrams);
        }
        const query = `
     ...
     */
    async getProductBySKU(sku, shopId) {
        const query = `
      SELECT * FROM products
     ...
     * Search products
     */
    async searchProducts(shopId, filters, pagination) {
        let query = `
      SELECT * FROM products
      WHERE shop_id = $1 AND deleted_at IS NULL
    `;
        ...
        // Count total
        const countResult = await this.db.query(query.replace('SELECT *', 'SELECT COUNT(*)'), values);
