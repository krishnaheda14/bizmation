...
export function paymentsRouter() {
    ...
    const handleCreateBuyOrder = async (req, res) => {
        try {
    ...
    const handleVerifyBuyPayment = async (req, res) => {
        try {
            ...
            sendTelegramAlert(`🚨 <b>NEW ${metal} ORDER (PAID)</b> 🚨\n\n` +
                `<b>Weight:</b> ${weight}g\n` +
                ...
                `<i>Please book/hedge this quantity immediately.</i>`).catch(() => { });
            return res.json({
                success: true,
                data: {
                    expired: false,
    ...
    // Gift endpoints
    router.post('/gift/lookup-user', async (req, res) => {
        try {
    ...
    router.post('/gift/transfer', async (req, res) => {
        try {
    ...
    // SIP setup
    router.post('/create-sip-subscription', async (req, res) => {
        try {
            ...
            if (!planId) {
                ...
                if (frequency === 'weekly')
                    period = 'weekly';
                const planResp = await axios.post('https://api.razorpay.com/v1/plans', {
                    period,
                    interval,
                    item: {
            ...
            const subResp = await axios.post('https://api.razorpay.com/v1/subscriptions', {
                plan_id: planId,
    ...
    router.post('/verify-subscription', async (req, res) => {
        try {
            ...
            sendTelegramAlert(`♻️ <b>NEW SIP (AUTOPAY) ACTIVATED</b> ♻️\n\n` +
                `<b>Subscription ID:</b> <code>${razorpaySubscriptionId}</code>\n` +
                `<b>Payment ID:</b> <code>${razorpayPaymentId}</code>\n\n` +
                `<i>A user has successfully set up a new Gold SIP. The first deduction is complete.</i>`).catch(() => { });
            return res.json({
                success: true,
                data: {
                    paymentId: razorpayPaymentId,
