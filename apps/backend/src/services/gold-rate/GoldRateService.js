...
export class GoldRateService {
     ...
     */
    async getCurrentRate(metalType, purity) {
        const query = `
        ...
        try {
            ...
            if (ageSeconds > ttlSeconds) {
                try {
                    ...
                    console.warn('[GoldRateService] Live fetch failed, returning cached DB rate:', err?.message || err);
     ...
     */
    async fetchLiveRate(metalType, purity) {
        return this.fetchAndSaveRate(metalType, purity);
    }
    /**
    ...
    async fetchFromAPI(metalType) {
        try {
            ...
            const platforms = swissquoteResponse.data;
            ...
            try {
                const erResponse = await axios.get('https://api.exchangerate.fun/latest?base=USD', { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; bizmation-gold-fetcher/1.0)', 'Accept': 'application/json' }, timeout: 8000 });
                ...
                const currencyResponse = await axios.get('https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/usd.json', { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; bizmation-gold-fetcher/1.0)', 'Accept': 'application/json' }, timeout: 10000 });
     ...
     * Fetch and save rate
     */
    async fetchAndSaveRate(metalType, purity) {
     ...
     */
    async updateRate(metalType, purity, ratePerGram) {
        // Deactivate old rates
        await this.db.query('UPDATE gold_rates SET is_active = false WHERE metal_type = $1 AND purity = $2', [metalType, purity]);
     ...
     * Get rate history
     */
    async getRateHistory(metalType, purity, days = 30) {
        const query = `
      SELECT * FROM gold_rates
    ...
    async autoUpdateRates() {
        ...
        const metalTypes = [MetalType.GOLD, MetalType.SILVER, MetalType.PLATINUM];
     ...
     */
    async reconcileAndUpdateAll(thresholdPercent = 0.5) {
        const metals = [MetalType.GOLD, MetalType.SILVER];
        ...
        for (const metal of metals) {
            ...
            try {
                ...
                console.warn('[GoldRateService] reconcile: failed to fetch upstream for', metal, err?.message || err);
                continue;
            }
            for (const purity of purities) {
                const purityMap = {
                ...
                if (shouldUpdate) {
                    ...
                    try {
                        ...
                        console.error('[GoldRateService] reconcile: failed to persist updated rate', e?.message || e);
                    }
                }
            }
        }
