...
const SlideToConfirm: React.FC<{
  label: string; value: number; disabled: boolean; paying: boolean;
  onChange: (v: number) => void; onConfirm: () => void;
}> = ({ label, value, disabled, paying, onChange, onConfirm }) => {
  ...
  return (
    <div className="relative select-none" ref={trackRef}>
      {/* Track */}
      <div
        className={`relative h-16 rounded-2xl overflow-hidden transition-all ${disabled && !paying ? 'opacity-50 cursor-not-allowed' : ''}`}
        style={{
          background: disabled && !paying
            ? 'linear-gradient(135deg,#9ca3af 0%,#a1a5aa 100%)'
            : confirmed
            ? 'linear-gradient(90deg,#16a34a,#22c55e)'
            : 'linear-gradient(135deg,#1c1c1c 0%,#2a2a2a 100%)',
          border: disabled && !paying
            ? '1.5px solid rgba(156,163,175,0.5)'
            : confirmed
            ? '1.5px solid #22c55e'
            : '1.5px solid rgba(251,191,36,0.5)',
          boxShadow: disabled && !paying
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none pl-14">
          <span className={`text-sm font-bold tracking-wide transition-colors ${
            disabled && !paying ? 'text-gray-600' : confirmed ? 'text-white' : 'text-amber-300 dark:text-yellow-400'
          }`}>
            {disabled && !paying ? 'Slide unavailable' : confirmed ? 'Confirmed — opening payment...' : label}
          </span>
        </div>
        ...
        >
          <div
            ...
            style={{
              ...
              color: disabled && !paying ? '#6b7280' : confirmed ? '#fff' : '#451a03',
              transform: disabled && !paying ? 'scale(1)' : 'scale(1.05)',
            }}
          >
            {disabled && !paying ? <svg viewBox="0 0 24 24" fill="currentColor" strokeWidth={2} className="w-5 h-5"><rect x="6" y="4" width="12" height="14" rx="2"/><path d="M9 14h6" stroke="currentColor" fill="none" strokeLinecap="round"/></svg> : confirmed ? <CheckSVG /> : <ChevronSVG />}
          </div>
        </div>
      ...
      <p className={`text-center text-xs mt-1.5 flex items-center justify-center gap-1 transition-colors ${
        disabled && !paying ? 'text-gray-400 dark:text-gray-600' : 'text-amber-500/70 dark:text-amber-400/70'
      }`}>
        ...
        {paying ? 'Processing payment...' : disabled ? 'Price lock expired' : 'Slide right to confirm purchase'}
      </p>
    </div>
  );
};
