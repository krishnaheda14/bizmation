...
async function getFirebaseAccessToken(env) {
    ...
    if (firebaseAccessTokenCache && firebaseAccessTokenCache.expiresAtMs - 60000 > nowMs) {
...
async function firestoreGetDoc(env, token, collection, docId) {
    ...
    if (!resp.ok)
        throw new Error(`Firestore read failed (${resp.status})`);
    return json;
}
async function firestoreRunPhoneQuery(env, token, phone) {
    ...
    const json = await resp.json().catch(() => ([]));
    if (!resp.ok)
        throw new Error(`Firestore query failed (${resp.status})`);
    if (!Array.isArray(json))
        return null;
    const found = json.find((row) => row?.document);
    return found?.document || null;
}
async function firestoreCreateDoc(env, token, collection, docId, data) {
...
async function handleGiftLookupUser(request, env, origin) {
...
async function handleGiftTransfer(request, env, origin) {
    const body = await request.json().catch(() => ({}));
...
async function handleCreateBuyOrder(request, env, origin) {
    const body = await request.json().catch(() => ({}));
    try {
...
async function handleVerifyBuyPayment(request, env, origin) {
    const body = await request.json().catch(() => ({}));
    ...
    try {
        ...
        await sendTelegramAlert(env, `🚨 <b>NEW ${record.metal || ""} ORDER (PAID)</b> 🚨\n\n` +
...
async function handleCreateSubscription(request, env, origin) {
    try {
        ...
        if (!planRes.ok)
            throw new Error('Razorpay failed to create plan');
        ...
        if (!subRes.ok)
            throw new Error('Razorpay failed to create subscription');
...
async function handleVerifySubscription(request, env, origin) {
    try {
        const body = await request.json().catch(() => ({}));
        ...
        await sendTelegramAlert(env, `♻️ <b>NEW SIP (AUTOPAY) ACTIVATED</b> ♻️\n\n` +
...
async function handleRedemptionNotify(request, env, origin) {
    const body = await request.json().catch(() => ({}));
    ...
    await sendTelegramAlert(env, `📉 <b>REDEMPTION ${event}</b>\n\n` +
        `<b>Status:</b> ${status}\n` +
